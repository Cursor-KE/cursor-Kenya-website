...
async function processLumaWebhookDelivery (
  deliveryId: string,
...
async function retryLumaWebhookDelivery (
  deliveryId: string,
  payload: LumaWebhookPayload,
  objectId: string | null
) {
  ...
  return processLumaWebhookDelivery(deliveryId, payload, objectId)
}
...
export async function processLumaWebhookBody (rawBody: string) {
  ...
  if (inserted.length === 0) {
    return retryLumaWebhookDelivery(deliveryId, payload, objectId)
  }
