...
async function insertDelivery (delivery: LumaWebhookDeliveryInput) {
...
async function getDeliveryStatus (id: string): Promise<StoredDeliveryStatus | null> {
  const [delivery] = await db
    .select({ status: lumaWebhookDeliveries.status })
    .from(lumaWebhookDeliveries)
    .where(eq(lumaWebhookDeliveries.id, id))
    .limit(1)

  return delivery?.status ?? null
}

async function markDeliveryProcessing (delivery: LumaWebhookDeliveryInput) {
  await db
    ...
    .where(eq(lumaWebhookDeliveries.id, delivery.id))
}

async function markDeliveryProcessed (id: string, status: DeliveryStatus) {
  await db
    .update(lumaWebhookDeliveries)
    .set({
      status,
...
export async function processLumaWebhookBody (rawBody: string) {
  return processLumaWebhookBodyWithDependencies(rawBody, {
    insertDelivery,
    getDeliveryStatus,
