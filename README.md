...
| `LUMA_WEBHOOK_SECRET` | Required in production unless `LUMA_WEBHOOK_ROUTE_TOKEN` is set | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Required in production unless `LUMA_WEBHOOK_SECRET` is set | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. |
