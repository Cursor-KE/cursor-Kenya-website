...
| `LUMA_WEBHOOK_SECRET` | Required for Luma webhooks | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. `/webhook` rejects POSTs when this is unset. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Required for Luma webhooks | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. `/webhook` rejects POSTs when this is unset. |
...
1. Configure `LUMA_API_KEY`, `NEXT_PUBLIC_APP_URL` or `LUMA_WEBHOOK_URL`, and `LUMA_WEBHOOK_ROUTE_TOKEN`.
