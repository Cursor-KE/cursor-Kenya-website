...
| `LUMA_WEBHOOK_SECRET` | Required for webhook POSTs unless route token is set | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Required for webhook POSTs unless signature secret is set | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. |
...
1. Configure `LUMA_API_KEY`, `NEXT_PUBLIC_APP_URL` or `LUMA_WEBHOOK_URL`, and either `LUMA_WEBHOOK_ROUTE_TOKEN` or `LUMA_WEBHOOK_SECRET` (the route rejects POSTs when neither credential is configured).
