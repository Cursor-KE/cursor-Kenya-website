...
| `LUMA_WEBHOOK_SECRET` | Recommended for Luma webhooks | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. At least one webhook verifier (`LUMA_WEBHOOK_SECRET` or `LUMA_WEBHOOK_ROUTE_TOKEN`) must be set for `/webhook` writes. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Recommended for Luma webhooks | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. At least one webhook verifier (`LUMA_WEBHOOK_SECRET` or `LUMA_WEBHOOK_ROUTE_TOKEN`) must be set for `/webhook` writes. |
