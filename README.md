...
| `LUMA_WEBHOOK_SECRET` | Required for webhook POST unless route token is set | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Required for webhook POST unless signature secret is set | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. |
