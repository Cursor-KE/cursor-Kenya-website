...
| `LUMA_WEBHOOK_SECRET` | Required for Luma webhooks unless `LUMA_WEBHOOK_ROUTE_TOKEN` is set | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Required for Luma webhooks unless `LUMA_WEBHOOK_SECRET` is set | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. |
...
1. Configure `LUMA_API_KEY`, `NEXT_PUBLIC_APP_URL` or `LUMA_WEBHOOK_URL`, and at least one webhook verifier: `LUMA_WEBHOOK_ROUTE_TOKEN` or `LUMA_WEBHOOK_SECRET`.
