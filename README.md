...
| `LUMA_WEBHOOK_SECRET` | Production webhook auth | Verifies Luma webhook signatures from `webhook-*` / `svix-*` headers using the returned `whsec_...` secret. In production, configure this or `LUMA_WEBHOOK_ROUTE_TOKEN`. |
| `LUMA_WEBHOOK_ROUTE_TOKEN` | Production webhook auth | Shared token accepted by `/webhook` through `?token=...`, `x-webhook-token`, or bearer auth. In production, configure this or `LUMA_WEBHOOK_SECRET`. |
...
1. Configure `LUMA_API_KEY`, `NEXT_PUBLIC_APP_URL` or `LUMA_WEBHOOK_URL`, and at least one production webhook auth value: `LUMA_WEBHOOK_SECRET` or `LUMA_WEBHOOK_ROUTE_TOKEN`.
