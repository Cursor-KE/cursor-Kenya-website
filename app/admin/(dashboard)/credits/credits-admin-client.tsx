    ...
    <Tabs defaultValue="campaigns">
      ...
      <TabsContent value="campaigns" className="mt-5 space-y-5">
        ...
        <div className="grid gap-5 xl:grid-cols-[380px_minmax(0,1fr)]">
          ...
          <Card className={!isSuperUser ? 'xl:col-span-2' : ''}><CardHeader><CardTitle>Campaign control</CardTitle><CardDescription>Campaign creation is super-admin-only; lifecycle management remains available to approved admins.</CardDescription></CardHeader><CardContent className="space-y-5"><CampaignPicker campaigns={campaigns} value={campaignId} onChange={setCampaignId} />{selectedCampaign ? <ActionForm action={updateCreditCampaign} submit="Update lifecycle"><input type="hidden" name="id" value={selectedCampaign.id} /><div className="grid gap-4 sm:grid-cols-3"><NativeSelect label="Status" name="status" defaultValue={selectedCampaign.status}>{['draft','active','paused','ended','archived'].map((status) => <option key={status} value={status}>{status}</option>)}</NativeSelect><Field label="Claims open" name="claimStartsAt" type="datetime-local" defaultValue={selectedCampaign.claimStartsAt?.slice(0,16)} /><Field label="Claims close" name="claimEndsAt" type="datetime-local" defaultValue={selectedCampaign.claimEndsAt?.slice(0,16)} /></div></ActionForm> : <p className="text-sm text-muted-foreground">No campaigns yet.</p>}</CardContent></Card>
        </div>
